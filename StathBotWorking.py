...
async def on_message(message):
    ...
    if message.channel.name == channelName:
        ...
        elif userMessage.lower()==f'{com}temp_func':
        ...
        #Flute command (leons request)

        elif userMessage.lower() == f'{com}flute':
            temp = ""
            for i in range(0,flu.number):
                temp+="u"
            await message.channel.send(f'fl{temp}te')
            flu.number+=1
            if flu.number==flu.max_val:
                flu.counter+=1
        ...
        #Command to get the number of questions a user has asked
            ...
            for line in lines:
                if str(message.author) in line:
                    await message.channel.send(username+" has answered "+line.strip('\n').split(',')[1]+" questions correctly!")
                    return
        ...
        elif userMessage.lower()==f'{com}endgame' and (message.author==q.start_player or message.author in admins_list):
        ...
        #Answer command (this needs needs to be the second last command in this if):
            ...
            found=False
            for i in range(0,len(lines)):
                if str(message.author) in lines[i]:
                    found=True
                    temp_num = i
            ...
            if found:
                lines[temp_num] = temp_line[0]+","+str(int(temp_line[1])+1)+"\n"
